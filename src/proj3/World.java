   ...
   public void toArray (List <Thing> ma) {
	...
	private void processFile(Scanner sc){
		...
		while (sc.hasNextLine()){
			...
			case "pship":
				PassengerShip pship = new PassengerShip(tScan, dockMap, portMap);
			...
			case "cship":
				CargoShip cship = new CargoShip(tScan, dockMap, portMap);
			...
			case "job":
				Job njob = new Job(tScan, shipMap);
				everyThing.add(njob);
				jobs.add(njob);        	
				assignJobToShip(njob, shipMap, dockMap);
				break;
	...
	void assignJobToShip(Job job, HashMap<Integer, Ship> sMap, HashMap<Integer, Dock> dMap){
		
		Dock nDock;
		Ship nShip = sMap.get(job.getParent());
		SeaPort port;
		
		// found ship
		if (nShip != null){
			nShip.getJobs().add(job);
			port = nShip.getPort();
			port.jobs.add(job);
		}
			...
			nDock = dMap.get(job.getParent());
			nDock.getShip().getJobs().add(job);
			port = ports.get(nDock.getParent());
		}
